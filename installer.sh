#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpTMDBSearch/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpTMDBSearch

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpTMDBSearch
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpTMDBSearch
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpTMDBSearch/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpTMDBSearch-main/usr' '/'
set +e
cd
sleep 2
...
echo ""
echo "#########################################################"
echo "#         CiefpTMDBSearch INSTALLED SUCCESSFULLY        #"
